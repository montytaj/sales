<x-app-layout>
    <x-slot name="breadcrumbs">
        @php
            $breadcrumbs = [
                ...
                ['label' => __('customers.create_customer')]
            ];
        @endphp
    </x-slot>
    ...
    <x-slot name="header">
        <h2 class="h4 mb-0 font-bold text-dark">
            <i class="bi bi-person-plus-fill text-primary me-2"></i>{{ __('customers.create_customer') }}
        </h2>
    </x-slot>
                    ...
                    <form method="POST" action="{{ route('customers.store') }}" enctype="multipart/form-data">
                        ...
                        <div class="row g-3 mb-3">
                            ...
                            <div class="col-12 col-md-4">
                                ...
                                <select name="category" id="category" class="form-select @error('category') is-invalid @enderror" required>
                                    <option value="regular" {{ old('category') === 'regular' ? 'selected' : '' }}>{{ __('customers.regular') }}</option>
                                    <option value="vip" {{ old('category') === 'vip' ? 'selected' : '' }}>{{ __('customers.vip') }}</option>
                                    <option value="corporate" {{ old('category') === 'corporate' ? 'selected' : '' }}>{{ __('customers.corporate') }}</option>
                                    <option value="wholesale" {{ old('category') === 'wholesale' ? 'selected' : '' }}>{{ __('customers.wholesale') }}</option>
                                </select>
                            </div>
                        ...
                        <div class="row g-3 mb-3">
                            <div class="col-12 col-md-6">
                                ...
                                <input type="number" step="0.01" name="credit_limit" id="credit_limit" class="form-control" value="{{ old('credit_limit', '0.00') }}" required>
                            </div>
                            ...
                            <div class="col-12 col-md-6">
                                ...
                                <input type="number" name="credit_period_days" id="credit_period_days" class="form-control" value="{{ old('credit_period_days', '0') }}" required>
                            </div>
                        </div>
                        ...
                        <div class="mb-4">
                            <label for="attachment" class="form-label font-semibold">{{ __('customers.upload_attachment') }}</label>
                        ...
                        <div class="d-flex justify-content-end gap-2">
                            ...
                            <button type="submit" class="btn btn-primary px-4">
                                <i class="bi bi-check-lg me-1"></i>حفظ العميل
                            </button>
                        </div>
                    </form>
                </div>
