<x-app-layout>
    <x-slot name="breadcrumbs">
        @php
            $breadcrumbs = [
                ...
                ['label' => __('services.edit_service')]
            ];
        @endphp
    </x-slot>
    ...
    <x-slot name="header">
        <h2 class="h4 mb-0 font-bold text-dark">
            <i class="bi bi-pencil-square text-primary me-2"></i>{{ __('services.edit_service') }}: {{ $service->name_ar }}
        </h2>
    </x-slot>
                    ...
                    <form method="POST" action="{{ route('services.update', $service) }}">
                        ...
                        <div class="row g-3 mb-3">
                            <div class="col-12 col-md-6">
                                ...
                                <input type="text" name="name_ar" id="name_ar" class="form-control @error('name_ar') is-invalid @enderror" value="{{ old('name_ar', $service->name_ar) }}" required>
                                @error('name_ar') <div class="invalid-feedback">{{ $message }}</div> @enderror
                            </div>

                            <div class="col-12 col-md-6">
                                <label for="name_en" class="form-label font-semibold">{{ __('services.name_en') }}</label>
                                <input type="text" name="name_en" id="name_en" class="form-control" value="{{ old('name_en', $service->name_en) }}">
                            </div>
                        </div>
                        ...
                        <div class="row g-3 mb-3">
                            <div class="col-12 col-md-4">
                                ...
                                <select name="service_type" id="service_type" class="form-select @error('service_type') is-invalid @enderror" required>
                                    @foreach (__('services.types') as $typeKey => $typeName)
                                        <option value="{{ $typeKey }}" {{ old('service_type', $service->service_type) === $typeKey ? 'selected' : '' }}>{{ $typeName }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="col-12 col-md-4">
                                ...
                                <div class="input-group">
                                    <input type="number" step="0.01" name="default_price" id="default_price" class="form-control" value="{{ old('default_price', $service->default_price) }}" required>
                            ...
                            <div class="col-12 col-md-4">
                                ...
                                <select name="unit_of_measure" id="unit_of_measure" class="form-select" required>
                                    @foreach (__('services.units') as $unitKey => $unitName)
                                        <option value="{{ $unitKey }}" {{ old('unit_of_measure', $service->unit_of_measure) === $unitKey ? 'selected' : '' }}>{{ $unitName }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="description" id="description" rows="3" class="form-control">{{ old('description', $service->description) }}</textarea>
                        </div>
                        ...
                        <div class="row g-3 mb-4">
                            <div class="col-12 col-md-6">
                                <div class="form-check form-switch p-3 bg-light rounded border">
                                    <input class="form-check-input ms-0 me-2" type="checkbox" name="is_taxable" value="1" id="is_taxable" {{ old('is_taxable', $service->is_taxable) ? 'checked' : '' }}>
                            ...
                            <div class="col-12 col-md-6">
                                <div class="form-check form-switch p-3 bg-light rounded border">
                                    <input class="form-check-input ms-0 me-2" type="checkbox" name="is_active" value="1" id="is_active" {{ old('is_active', $service->is_active) ? 'checked' : '' }}>
