    ...
    <x-slot name="header">
        <div class="d-flex justify-content-between align-items-center">
            <h2 class="h4 mb-0 font-bold text-gray-800">
                <i class="bi bi-pencil text-primary me-2"></i>تعديل الصنف: {{ $item->name }}
            </h2>
                    ...
                    <div class="card-body p-4">
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" name="code" class="form-control" value="{{ $item->code ?? $item->item_code }}">
                            </div>
                            <div class="col-md-6">
                        ...
                        <div class="row g-3">
                            <div class="col-md-4">
                                ...
                                <select name="base_unit_id" class="form-select" required>
                                    ...
                                    @foreach($units as $u)
                                        <option value="{{ $u->id }}" {{ $item->base_unit_id == $u->id ? 'selected' : '' }}>{{ $u->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-4">
                                ...
                                <select name="wholesale_unit_id" class="form-select">
                                    ...
                                    @foreach($units as $u)
                                        <option value="{{ $u->id }}" {{ $item->wholesale_unit_id == $u->id ? 'selected' : '' }}>{{ $u->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-4">
                                <label class="form-label font-bold">معامل التحويل (عدد الوحدات الصغرى) <span class="text-danger">*</span></label>
                                <input type="number" step="0.01" name="conversion_factor" class="form-control" value="{{ $item->conversion_factor }}" required>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-md-4">
                <div class="card shadow-sm border-0 rounded-3 p-4">
                    <div class="form-check form-switch mb-3">
                        <input class="form-check-input" type="checkbox" name="is_active" id="activeItemSwitch" {{ $item->is_active ? 'checked' : '' }}>
                    ...
                    <button type="submit" class="btn btn-primary btn-lg w-100 py-3 font-bold">
                        <i class="bi bi-check-circle me-1"></i>تحديث بيانات الصنف
                    </button>
                </div>
            </div>
        </div>
