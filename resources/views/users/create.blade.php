<x-app-layout>
    ...
    <x-slot name="header">
        <h2 class="h4 mb-0 font-bold text-dark">
            <i class="bi bi-person-plus-fill text-primary me-2"></i>{{ __('users.create_user') }}
        </h2>
    </x-slot>
                    ...
                    <form method="POST" action="{{ route('users.store') }}">
                        ...
                        <div class="row g-3 mb-3">
                            <div class="col-12 col-md-6">
                                <label for="password" class="form-label font-semibold">{{ __('users.password') }} <span class="text-danger">*</span></label>
                            ...
                            <div class="col-12 col-md-6">
                                <label for="password_confirmation" class="form-label font-semibold">{{ __('users.password_confirmation') }} <span class="text-danger">*</span></label>
                                ...
                                @foreach ($roles as $role)
                                    ...
                                    <div class="col-12 col-md-4">
                                        <div class="form-check p-2 bg-white rounded border">
                                            <input class="form-check-input user-role-cb" type="checkbox" name="roles[]" value="{{ $role->name }}" id="role_{{ $role->id }}" {{ is_array(old('roles')) && in_array($role->name, old('roles')) ? 'checked' : '' }}>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="main_branch_id" id="main_branch_id" class="form-select mb-3">
                                ...
                                @foreach ($branches as $branch)
                                    <option value="{{ $branch->id }}" {{ old('main_branch_id') == $branch->id ? 'selected' : '' }}>
                            ...
                            <div class="row g-2 p-3 bg-light rounded border">
                                @foreach ($branches as $branch)
                                    <div class="col-12 col-md-6">
                                        <div class="form-check p-2 bg-white rounded border">
                                            <input class="form-check-input user-branch-cb" type="checkbox" name="branches[]" value="{{ $branch->id }}" id="branch_{{ $branch->id }}" {{ is_array(old('branches')) && in_array($branch->id, old('branches')) ? 'checked' : '' }}>
